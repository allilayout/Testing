...
function Get-IPv4Mask([int]$PrefixLength) {
    if ($PrefixLength -lt 0 -or $PrefixLength -gt 32) {
        throw "Invalid CIDR prefix: /$PrefixLength"
    }
    if ($PrefixLength -eq 0) { return [uint32]0 }

    # In Windows PowerShell, the hexadecimal literal 0xFFFFFFFF is the signed
    # Int32 value -1. Casting that literal to UInt64 throws. Start with the
    # explicitly unsigned UInt32 maximum instead, then widen before shifting.
    $allIPv4Bits = [uint64]([uint32]::MaxValue)
    return [uint32](($allIPv4Bits -shl (32 - $PrefixLength)) -band $allIPv4Bits)
}
...
function Expand-Cidr([string]$cidr) {
    ...
    $prefix = 0
    if (-not [int]::TryParse($parts[1].Trim(), [ref]$prefix)) { throw "Invalid CIDR prefix: /$($parts[1])" }
    $mask = Get-IPv4Mask $prefix
    $base = ConvertTo-UInt32IP $parts[0].Trim()
    $network = [uint32]($base -band $mask)
    # Keep address counts in UInt64 throughout. This represents /0's 2^32
    # addresses exactly and avoids routing integral CIDR math through Double.
    $total = ([uint64]1 -shl (32 - $prefix))
...
function Expand-Target([string]$spec) {
    ...
    foreach ($chunk in ($spec -split ',')) {
        ...
        else { $null = ConvertTo-UInt32IP $c; $result.Add($c) }
    }
